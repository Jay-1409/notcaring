...
def main(args):
    ...
    if args.pix_init == 'real':
        ...
        for c in range(num_classes):
            if len(indices_class[c]) < args.ipc:
                print(f"⚠️  Warning: Not enough real images for class {c}. Needed: {args.ipc}, Found: {len(indices_class[c])}")
                continue  # Skip or handle gracefully

            imgs = get_images(c, args.ipc).detach()  # [ipc, H, W] or [ipc, C, H, W]

            # Add channel dim if needed
            if imgs.ndim == 3:
                imgs = imgs.unsqueeze(1)  # [ipc, 1, H, W]

            # Resize if needed
            if imgs.shape[2:] != (im_size[0], im_size[1]):
                imgs = F.interpolate(imgs, size=im_size, mode='bilinear', align_corners=False)

            if imgs.shape[0] != args.ipc:
                print(f"❌ Skipping class {c} due to shape mismatch: got {imgs.shape[0]}, expected {args.ipc}")
                continue  # Avoid assignment if shape mismatch
