...
class MMFi_Dataset(Dataset):
    ...
    def load_data(self):
        ...
        for subject, actions in self.data_source.items():
            for action in actions:
                print(f"Loading data for subject: {subject}, action: {action}")  # Debugging line
                if self.data_unit == 'sequence':
                    ...
                    for mod in self.modality:
                        data_dict[mod+'_path'] = os.path.join(self.data_base.data_root, self.get_scene(subject), subject,
                                                             action, mod)
                        print(f"Modality path for {mod}: {data_dict[mod+'_path']}")  # Debugging line
                    ...
                    print(f"Loading data from path: {data_path}")  # Debugging line
                    try:
                        mat_files = [f for f in os.listdir(data_path) if f.endswith('.npy')]
                        frame_num = len(mat_files)  # Dynamic frame count
                        print(f"Found {frame_num} frames in {data_path}")  # Debugging line
                    except FileNotFoundError as e:
                        print(f"Error: {e}. Directory not found: {data_path}")  # Error handling
                        continue  # Skip to the next action/subject
                    ...
                    for idx in range(frame_num):
                        ...
                        for mod in self.modality:
                            ...
                            # Debugging each file check
                            print(f"Checking file path for {mod}: {data_dict[mod+'_path']}")
                            
                            if not os.path.exists(data_dict[mod+'_path']):
                                print(f"File not found: {data_dict[mod+'_path']}")  # Debugging line
                                data_valid = False
                            elif os.path.getsize(data_dict[mod+'_path']) == 0:
                                print(f"File is empty: {data_dict[mod+'_path']}")  # Debugging line
                                data_valid = False
                        if data_valid:
                            print(f"Valid data for subject {subject}, action {action}, frame {idx}")  # Debugging line
                            data_info.append(data_dict)
                        else:
                            print(f"Invalid data for subject {subject}, action {action}, frame {idx}")  # Debugging line
                else:
                    raise ValueError('Unsupported data unit!')
        
        print(f"Loaded {len(data_info)} data samples.")  # Debugging line
        return data_info
...
class WiFiDataAugmentation:
    def __init__(self, noise_std=0.01, max_shift=5, scale_range=(0.9, 1.1)):
    ...
    def __call__(self, data):
        ...
        data = self.scale_amplitude(data)
        return data
...
def make_dataset(dataset_root, config):
    print(f"Accessing dataset at: {dataset_root}")
    print(f"Files in dataset: {os.listdir(dataset_root)}")

    database = MMFi_Database(dataset_root)
    config_dataset = decode_config(config)
    wifi_augmentation = WiFiDataAugmentation(noise_std=0.01, max_shift=2, scale_range=0.1)
